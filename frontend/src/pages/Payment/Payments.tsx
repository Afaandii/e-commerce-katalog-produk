...
type Payments = {
  ...
  transaction?: { id: number; transaction_code: string };
};
...
export default function Payments() {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:8000/api/v1/get-payment", {
        headers: {
              ...
              <table className="min-w-full divide-y divide-gray-600">
                ...
                <tbody className="bg-gray-800 divide-y divide-gray-600">
                  {payment.map((pay, index) => (
                    <tr key={pay.id} className="hover:bg-gray-700">
                      <td className="px-4 py-3 text-white">{index + 1}</td>
                      <td className="px-4 py-3 text-white">
                        {pay.transaction?.transaction_code}
                      </td>
                      <td className="px-4 py-3 text-gray-300">
                        {pay.midtrans_transaction_id}
                      </td>
                      <td className="px-4 py-3 text-gray-300">
                        {pay.payment_type}
                      </td>
